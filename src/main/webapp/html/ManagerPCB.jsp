...
<script type="text/javascript" src="https://cdn.datatables.net/1.10.25/js/jquery.dataTables.min.js"></script>
    ...
    <script>
		...
		$("#myDataTalbe").on("click", ".btn-warning", function () {
			...
			var cno = tr.find('td:eq(1)').text();
			var name = tr.find('td:eq(2)').text();
			var date = tr.find('td:eq(3)').text();
			var isalive = tr.find('td:eq(4)').text();
			var outdate = tr.find('td:eq(5)').text();
            ...
            $(this).parents('tr').before("<tr>\
				<td><input type='text' size='4' value=" + key + " ></td>\
				<td><select>\
                    <option selected='selected'>"+ cno +"</option>\
                    <option>A01</option>\
                    <option>A02</option>\
                    <option>B01</option>\
                    <option>B02</option>\
				...
				<td><select>\
                    ...
                    <option>極速SERVERPCB-2021</option>\
                    <option>超極速SERVERPCB-2021</option>\
                    <option>M03系PCB-2107</option>\
                    <option>M03+系PCB-2018</option>\
                    <option>高工SERVERPCB</option>\
                    </select>\
                    </td>\
				<td><input type='text' size='2' value=" + date + " ></td>\
				<td><select>\
                    <option>Y</option>\
                    <option>N</option>\
                    </select>\
		...
		$("#myDataTalbe").on("click", ".btn-success", function () {
			...
			var name1 = tr1.find('td:eq(4)').find("select").val();
